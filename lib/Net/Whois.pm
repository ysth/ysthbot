...
sub new {
  ...
  @_ == 1 or @_ == 2 or croak "usage: new $class DOMAIN";
  my ($domain,$nosock) = @_;
  my $text;
  my $retval;
...
# }
    ...
    if (/Registrant/) {
      ...
      $info{'MATCH'} = 0;
      # fall back on whois command
      if (! $nosock) { %info = (%info,%{($class->new($domain, 1)||[{}])->[0]}); return } 
    }
    if ($info{'MATCH'} ) { 
    @info{'NAME', 'TAG'} = (	      $t =~ /^(.*)(?:\s+\((\S+)\))?$/)
    or die "Registrant Name not found in returned information: '$t'";
