...
sub fallback {
  ...
  if ( $body =~ s/\s*\?+$// and ( $mess->{address} or $self->get("user_passive_ask") ) ) {
    ...
    # if there's no such factoid, is this a yes or no question?
    elsif ( $body =~ s/^(?i:is|are)\s+// ) {
      my $key;
      ($is_are, $key, $factoid) = $self->confirm_factoid($body);
      if (! $key) {
         return $mess->{address} ? "Dunno.  You tell me." : undef;
      }
      else {
         return "Yup, $key $is_are $factoid."
      }
    }
    else {
  ...
  if ($last_auto_forget && $body =~ $self->get("user_unautoforget")) {
    my $object = $last_auto_forget->[0];
    $self->add_factoid($object, $last_auto_forget->[1], split(/\s+or\s+/, $last_auto_forget->[2]) );
    undef $last_auto_forget;
    my ($is_are, $current) = $self->get_factoid($object);
    return "ok, $object $is_are $current";
  }
  ...
  # warn with old value when replacing
  my $message;
  if (!$also) {
    # get any current factoid there might be.
    my ($was_were, $current) = $self->get_factoid($object);
    if ($current) {
      $self->delete_factoid($object);
      $last_auto_forget = [ $object, $was_were, $current ];
      $was_were =~ y/iaIA/aeAE/;
      $message = "But I thought $object w\L$was_were\E $current";
    }
  }

  $self->add_factoid($object, $is_are, split(/\s+or\s+/, $description) );

  # return warning message or, if we were addressed, an ack
  return $message || ($mess->{address} ? "okay." : 1);
}

sub confirm_factoid {
  my ($self, $object) = @_;

  # try each chunk of whitespace in turn as the separator between
  # subject and object (so "is building brick red" parses as both
  # "is" "building" "brick red" and "is" "building brick" "red")
  while ( $object =~ /\s+/g ) {
    my $key = substr($object, 0, $-[0]);
    my $text = lc substr($object, $+[0]);
    $text =~ tr/a-z0-9//cd;
    my $text_re = qr!@{[join "\\W*", "^", split(//, $text), "\\z"]}!;
    my ($is_are, @factoids) = $self->get_raw_factoids($key);
    for (@factoids) {
      return ($is_are, $key, $_->{text}) if $_->{text} =~ $text_re;
    }
  }
...
sub get_raw_factoids {
  ...
  # expose our variables
  my $raw = $self->get( "user_".lc($object) );
  if ( $raw ) {
    @factoids = map +{ alternate => 0, text => $_ }, split /,/, $raw;
    $is_are = @factoids == 1 ? "is" : "are";
    return( $is_are, @factoids );
  }

  $raw = $self->get( "infobot_".lc($object) )
    or return ();
