...
sub said { 
    ...
    if ($body =~ /(good(\s+fuckin[\'g]?)?\s+(bo(t|y)|g([ui]|r+)rl))|(bot(\s|\-)?(?:snack|beer))/i) {
        if (rand()  < .5)  {
            return "thanks $who :)";
         } else {
            return ":)";
    ...
    if ($body =~ /\b(?:shaddup|shut\s*up)\b/) {
        return( ( "/me looks hurt", "who, me?", "Yessir!" )[rand 3] );
    }

    if ($body =~ /ysthbot!/) {
        return "Your Servant, Sir or Madam!";
    }

    if ($body =~ /(\bayb\b)|(all your base)|(all your \w+ belong)|(someone set us up the bomb)|(^\s*hello gentlemen\s*$)/i) {
        return "$who - All Your Base references are the bees' knees, the kittens' whiskers, the monkeys' cajones";
    }

    if ($body =~ /^\s*(h(ello|i( there)?|owdy|ey|ola)|salut|bonjour|niihau|que\s*tal)(\W+$nick)?\s*$/i) {
    ...
    if ($body =~ /^\s*signoff$/) {
        $self->say(channel => "#spqr", body => "/me wanders off into the sunset to look for a new home");
    }
